...
| Shoreline data | GSHHG Intermediate 2.3.7 installed. An offshore route executed 581 shoreline checks. A temporary north-to-south route across Anglesey completed around land; the engine reported 44,549 shoreline checks and 5,858 land rejections. A separate destination placed on land was rejected before the search started. |
...
The temporary Anglesey route and positions were removed from the tablet after
the test; its saved routing configuration was restored byte for byte. The
Android changes are guarded by `__OCPN__ANDROID__`; desktop layouts keep their
existing controls. Before publication, repeat this workflow with a production
OpenCPN Android package and a second screen size.
